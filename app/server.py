...
from app.services.claims_service import ClaimNotFound, ClaimsService, InvalidTransition
...
<select id="actorsel"><option>provider-1</option><option>reviewer-1</option><option>supervisor-1</option></select></p>
...
<input name="procedure_code" value="99214" placeholder="procedure"> <input name="amount" value="1250" placeholder="amount" type="number" step="0.01"> <button>Submit claim</button></form>
<div class="msg" id="msg"></div>
<table><thead><tr><th>Claim</th><th>Provider</th><th>Amount</th><th>Status</th><th>AI</th><th>Actions</th></tr></thead><tbody id="rows"></tbody></table>
<script>
...
async function load(){const c=await api('/claims');$('#rows').innerHTML=c.map(x=>`<tr><td><code>${x.claim_id}</code></td><td>${x.provider_id}</td><td>${x.amount.toFixed(2)}</td><td>${x.status}</td><td>${x.ai_decision||''}${x.ai_confidence?' ('+x.ai_confidence+')':''}</td>
<td>${x.status==='SUBMITTED'?`<button class=s onclick="act('${x.claim_id}','adjudicate')">Adjudicate</button>`:''}
${x.status==='AI_APPROVED'?`<button class=s onclick="act('${x.claim_id}','approve')">Approve</button>`:''}
        ...
        try:
            if path == "/claims":
                claim = self.service.submit_claim(
                    ...
                    data.get("procedure_code", "00000"), data.get("amount", 0))
